      ...
      <div className="space-y-6 p-5 sm:p-8">
        ...
        <section className="cartao rounded-lg p-5">
          <div className="flex items-start gap-3">
            <FileBarChart className="mt-0.5 size-5 text-mrg-600" />
            <div>
              <h2 className="font-display text-base font-bold text-tinta">Como o cliente recebe</h2>
          ...
          <ul className="mt-5 grid gap-2 sm:grid-cols-2">
            {BLOCOS.map((b) => (
              <li key={b} className="flex items-center gap-2 rounded-md border border-borda bg-nevoa px-3 py-2.5 text-sm text-grafite">
                <span className="size-1.5 rounded-full bg-mrg-600" />
                {b}
              </li>
            ))}
          </ul>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {DEMO_CLIENTES.filter((c) => c.status === "ativo").map((c) => (
            <article key={c.id} className="cartao rounded-lg p-5">
              <div className="flex items-start justify-between gap-3">
                <h3 className="font-semibold text-tinta">{c.nome}</h3>
                <Etiqueta tom="azul">mensal</Etiqueta>
              </div>
              <p className="mt-1 text-xs text-cinza">Próximo envio: dia 1º · por e-mail e link</p>
